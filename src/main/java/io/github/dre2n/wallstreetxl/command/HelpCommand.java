...
public class HelpCommand extends DRECommand {
    ...
    public HelpCommand() {
        ...
        setMinArgs(0);
        setMaxArgs(1);
        setHelp(WMessage.HELP_HELP.getMessage());
    ...
    public void onExecute(String[] args, CommandSender sender) {
        Set<DRECommand> wCommandList = plugin.getCommandCache().getCommands();
        ...
        int max = 0;
        int min = 0;
        for (DRECommand wCommand : wCommandList) {
            send++;
        ...
        for (DRECommand wCommand : toSend) {
            MessageUtil.sendMessage(sender, "&b" + wCommand.getCommand() + "&7 - " + wCommand.getHelp());
        }
    }
